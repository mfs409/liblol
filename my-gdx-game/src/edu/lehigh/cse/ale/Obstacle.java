 ...
 * 
 * There are many flavors of obstacles. They can have a _physics shape that is circular or square. They can have default
 * collision behavior or custom behavior. They can be moved by dragging. They can move by touching the object and then
 * touching a point on the screen. They can have "damp" behavior, which is a way to do tricks with Physics (such as zoom
 * strips or friction pads). A method for drawing bounding boxes on the screen is also available, as is a means of
 * creating "trigger" obstacles that cause user-specified code to run upon any collision. There is also a simple object
...
public class Obstacle extends PhysicsSprite
{
     ...
     * 
     * This should never be invoked directly. Instead, use the 'addXXX' methods of the Object class.
     * 
     * @param x
    ...
    /**
     * Call this on an Obstacle to rotate it. Note that this works best on boxes.
     * 
     * @param rotation
    ...
    /**
     * Track if the obstacle has an active "dampening" factor for custom _physics tricks
     */
    boolean _isDamp;
     ...
     * 
     * A hero can glide over damp Obstacles. Damp factors can be negative to cause a reverse direction, less than 1 to
     * cause a slowdown (friction pads), or greater than 1 to serve as zoom pads.
     * 
     * @param factor
     *            Value to multiply the hero's velocity when it is on this Obstacle
     */
    ...
    /**
     * Call this on an event to make it behave like a "damp" obstacle, except with a constant additive (or subtractive)
     ...
     * @param boostDuration
     *            How long should the speed boost last (use -1 to indicate "forever")
     */
    ...
    /**
     * Track if this is a "trigger" object that causes special code to run upon any collision with a hero
     */
    ...
    /**
     * Track if this is a "trigger" object that causes special code to run upon any collision with an enemy
     */
    ...
    /**
     * Track if this is a "trigger" object that causes special code to run upon any collision with a projectile
     */
    ...
    /**
     * Make the object a trigger object, so that custom code will run when a hero runs over (or under) it
     * 
     * @param activationGoodies
     *            Number of goodies that must be collected before this trigger works
     * @param id
     ...
     *            Number of type-1 goodies that must be collected before this trigger works
     * @param activationGoodies2
     *            Number of type-2 goodies that must be collected before this trigger works
     * @param activationGoodies3
     *            Number of type-3 goodies that must be collected before this trigger works
     * @param activationGoodies4
     *            Number of type-4 goodies that must be collected before this trigger works
     */
    public void setHeroCollisionTrigger(int id, int activationGoodies1, int activationGoodies2, int activationGoodies3, int activationGoodies4)
    {
        _heroTriggerID = id;
        _isHeroCollideTrigger = true;
    ...
    /**
     * Make the object a trigger object, so that custom code will run when a enemy runs over (or under) it
     * 
     * @param activationGoodies
     *            Number of goodies that must be collected before this trigger works
     * @param id
     ...
     *            Number of type-4 goodies that must be collected before this trigger works
     */
    public void setEnemyCollisionTrigger(int id, int activationGoodies1, int activationGoodies2, int activationGoodies3, int activationGoodies4)
    {
        _enemyTriggerID = id;
        _isEnemyCollideTrigger = true;
    ...
    /**
     * Set a delay between when an enemy collides with this trigger, and when this trigger actually executes
     * 
     * @param delayDuration
    ...
    /**
     * Make the object a trigger object, so that custom code will run when a projectile hits it.
     * 
     * @param activationGoodies
     *            Number of goodies that must be collected before this trigger works
     * @param id
     ...
     *            Number of type-4 goodies that must be collected before this trigger works
     */
    public void setProjectileCollisionTrigger(int id, int activationGoodies1, int activationGoodies2, int activationGoodies3, int activationGoodies4)
    {
        _projectileTriggerID = id;
        _isProjectileCollideTrigger = true;
     ...
     *            identifier for the trigger.
     * @param activationGoodies1
     *            Number of type-1 goodies that must be collected before it works
     * @param activationGoodies2
     *            Number of type-2 goodies that must be collected before it works
     * @param activationGoodies3
     *            Number of type-3 goodies that must be collected before it works
     * @param activationGoodies4
     *            Number of type-4 goodies that must be collected before it works
     */
    public void setTouchTrigger(int id, int activationGoodies1, int activationGoodies2, int activationGoodies3, int activationGoodies4)
    {
        _touchTriggerID = id;
        _isTouchTrigger = true;
    ...
    /**
     * When a _sprite is poked, we record it here so that we know who to move on the next screen touch
     */
    protected static Obstacle    _currentPokeSprite;

    /**
     * Rather than use a Vector2 pool, we'll keep a vector around for all poke operations
     */
    ...
    /**
     * When a _sprite is poked, remember the time, because rapid double-clicks cause deletion
     */
    private static float         _lastPokeTime;

    /**
     * Tunable constant for how much time between pokes constitutes a "double click"
     */
     ...
     * 
     * Poke the Obstacle, then poke the screen, and the Obstacle will move to the location that was pressed. Poke the
     ...
     * @param moveable
     *            Can the individual items that are drawn move on account of collisions?
     */
    ...
    /**
     * Code to handle the processing of a scribble event. Whenever we have a scribble event, we will draw an obstacle on
     * the scene. Note that there are some hard-coded values that should become parameters to setScribbleMode()
     * 
     * @param event
    ...
    private static void doScribble(/*final TouchEvent event*/)
    {
        ...
        if (o != null) {
            ...
            TimerHandler th = new TimerHandler(_scribbleTime, false, new ITimerCallback()
            {
                @Override
    ...
    /**
     * Indicate that when the player touches this obstacle, we should make a sound
     * 
     * @param sound
     *            The name of the sound file to play
     */
     ...
     * 
     * Note: This Obstacle is /never/ the dominant obstacle in a collision, since it is #6 or #7
     * 
     * @param other
     ...
     */
    /**
     * When the scene is touched, we use this to figure out if we need to move a PokeObject
     * 
     * @param scene
            ...
            switch (event.getAction()) {
                case TouchEvent.ACTION_DOWN:
                    if (_currentPokeSprite != null) {
                        ...
                        // move the object
                        _pokeVector.set(event.getX() / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT, event.getY()
                                / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT);
                        _currentPokeSprite._physBody.setTransform(_pokeVector, _currentPokeSprite._physBody.getAngle());
    ...
    protected boolean onSpriteAreaTouched(/*TouchEvent e, float x, float y*/)
    {
        ...
        if (_isTouchTrigger && e.isActionDown()) {
            ...
            {
                remove(false);
                ALE._self.onTouchTrigger(_touchTriggerID, MenuManager._currLevel, this);
                return true;
            }
        }
    ...
    /**
     * Store an obstacle that is the peer of this obstacle. This is useful when we want to allow one obstacle's trigger
