...
public class Tilt {
	...
	static void handleTilt() {
		...
		if (appType == ApplicationType.Android
			...
			// TODO: test this code in portrait mode, and test if upside-down screens work (landscape and portrait)
		...
		synchronized (_accelEntities) {
			if (_tiltVelocityOverride) {
				...
				if (_xGravityMax == 0) {
					...
					for (PhysicsSprite gfo : _accelEntities) {
						if (gfo._physBody.isActive())
							gfo.updateVelocity(
									gfo._physBody.getLinearVelocity().x,
									yGravity);
					}
					...
					for (PhysicsSprite gfo : _accelEntities) {
						if (gfo._physBody.isActive())
							gfo.updateVelocity(xGravity,
									gfo._physBody.getLinearVelocity().y);
					}
				} else {
					// Send the new gravity information to the _physics system
					// by
				...
				for (PhysicsSprite gfo : _accelEntities) {
					if (gfo._physBody.isActive())
						gfo._physBody.applyForce(_oacVec,
								gfo._physBody.getWorldCenter(), true);
