...
namespace LibLOL
{
    public class Svg
    {
         ...
         */
        internal class SVGSprite : PhysicsSprite
        {
            internal SVGSprite(String imgName, float width, float height)
                : base(imgName, width, height)
            {

            }

            internal override void OnCollide(PhysicsSprite other, FarseerPhysics.Dynamics.Contacts.Contact contact)
            {
            }
        }
        ...
        private void AddLine(float x1, float y1, float x2, float y2)
        {
            ...
            Body bd = BodyFactory.CreateEdge(Level.sCurrent.mWorld, new Vector2(-len / 2, 0), new Vector2(len / 2));
            bd.SetTransform(new Vector2(centerX, centerY), (float)Math.Atan2(y2 - y1, x2 - x1));

            SVGSprite invis = new SVGSprite("", len, 0.1f);
            invis.mBody = bd;
            bd.UserData = invis;
        ...
        {
            Stream stream = TitleContainer.OpenStream("Content\\" + svgName);
            XDocument doc = XDocument.Load(stream);

            IEnumerable<XElement> gs = doc.Elements("g");

            foreach (XElement g in gs)
            {
                XAttribute xform = g.Attribute("transform");
                if (xform != null)
                {
                    ProcessTransform(xform.Value);
                }
